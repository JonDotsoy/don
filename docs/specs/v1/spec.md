...
3. [Examples](#3-examples)
   ...
   - [Repeated Directive with Differing Argument Types](#36-repeated-directive-with-differing-argument-types)
   - [Multiple Top-Level Directives](#37-multiple-top-level-directives)
...
### 3.6 Repeated Directive with Differing Argument Types
...
The same directive name (`route`) repeats at the document root with a
second argument of a different type each time (`200`, a number, vs.
`true`, a boolean) — DON places no constraint linking one occurrence's
argument types to another's.

### 3.7 Multiple Top-Level Directives
...
The document root can hold any number of directives, of any names, one
after another — here a `server` block and an unrelated `route` block sit
