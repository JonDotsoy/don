...
### Block-scoped variables (`scopedVariablesPlugin`)

`variablesPlugin` above keeps one flat, document-wide `ctx` — a nested `set` permanently overwrites an outer one, since `onDirective` alone has no signal for when a block's children are done being visited to restore a shadowed value. `donly/plugins/scoped-variables` solves that with a **block-scoped** `set`: a `set` inside a block shadows one of the same name from an enclosing block only for the rest of that inner block, then reverts once the block ends — the same way a `let`/`var` is scoped to its block in most programming languages. `scopedVariablesPlugin` is a real `DonPlugin`, so it drops straight into `DON.parse()`'s own `plugins` array, built on a new hook, `DonPlugin#afterChildren` (see [Plugins](./docs/concepts/plugins.md)), that fires once a directive's children are all done being visited — exactly the "block ended" signal needed to pop a scope pushed in `onDirective`:
...
`tar`'s inner `set foo 55` only reaches `foo $foo` inside `tar`'s own block — the top-level `foo $foo` above it still resolves against the outer `set foo 33`. Unlike `variablesPlugin`, a bare `$name` argument keeps its bound value's original type (`33` stays a `number`, not `"33"`), and `${name}` interpolates inside a larger string argument (`"${project}-container-1"`), with `\${name}` escaping to the literal text `${name}`. `donly/plugins/scoped-variables` also exports `resolveScopedVariables`, the same rules applied as a standalone function over a `Directive` tree that already exists (from `DirectiveJSONDecoder`, or built by hand) rather than one `DON.parse()` is about to build. See [Scoped Variables](./docs/plugins/scoped-variables.md) for the full walkthrough, when to reach for which of the two, the error cases they throw on, and a known limitation (neither can currently tell a single- from a double-quoted string, so `${name}` interpolates inside either).
